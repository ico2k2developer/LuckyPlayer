...
public class MediaManager
{
    ...
    public void scan(Uri[] uris,@Nullable AsyncTask.OnStart start,@Nullable OnScanProgress progress,@Nullable AsyncTask.OnFinish<Long> finish)
    {
        ...
        final String[] keys = getBaseColumns();
            ...
            public Long call(@NonNull AsyncTask.PublishProgress<int[]> callback) throws Exception
            {
                ...
                for (tableId = 0; tableId < uris.length; tableId++) {
                    ...
                    else
                    {
                        ...
                        {
                            CursorResult result = getRealPath(context,cursor,columns);
                            itemId = Integer.parseInt(result.getData().get(_ID));
                            try
                            {
                                File newFile = result.loadBest(tableId,itemId);
                                Log.d(TAG, "Using path " + newFile.getUri());
                                ...
                                if (write) {
                                    SongDetailed song = SongDetailed.loadFrom(tableId,itemId,newFile);
                                    if (song != null) {
    ...
    public static class CursorResult
    {
        ...
        @Nullable
        public File loadBest(int tableId, int itemId)
        {
            File result = null;
            if (path != null) {
                try {
    ...
    private static Map<String,Integer> getBaseColumnIndexes(Cursor cursor,String[] columns,@Nullable Map<String,Integer> recycle)
    {
        if(recycle == null)
    ...
    private static Map<String,Integer> getBaseColumnIndexes(Cursor cursor,String[] columns)
    {
        return getBaseColumnIndexes(cursor,columns,null);
    }

    @Nullable
    private static CursorResult getRealPath(Context context,Cursor cursor,Map<String,Integer> indexes)
    {
        if(cursor != null)
        {
    ...
    @Nullable
    private static CursorResult getRealPath(Context context,Cursor cursor,String[] columns)
    {
        return getRealPath(context,cursor,getBaseColumnIndexes(cursor,columns));
    }
